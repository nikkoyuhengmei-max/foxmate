...
```bash
aquant universe --universe hs300 --refresh       # 拉取并生成缓存
aquant universe --universe hs300                 # 检查数量+前10只（应≈300）
aquant universe --universe zz500 --refresh       # ≈500
aquant universe --universe all --refresh         # 全A（数千只）
aquant universe --universe hs300 --source akshare# 指定数据源
aquant screen --strategy predictive_ranking --top 20 --universe hs300
```

- 加载顺序：**本地缓存 → AkShare → Baostock**；三者都失败则**报明确错误，不回退到小样本池**。
- 缓存到 `data/cache/universe/{hs300,zz500,sz50,all_a}.csv`（字段 `symbol,name,exchange,industry,source,updated_at`）。
- 数量门槛：hs300<250 / zz500<450 / all<1000 / watchlist<1 时**只报错、不运行模型**：
  「股票池数量异常，当前仅 N 只…」。
- **分层过滤诊断**：选股返回 `原始数量 → 基础过滤(剔除ST/停牌/退市) → 流动性过滤 → 最终入选` 每步数量；
  为空时给出具体原因（数据源没返回/行情未返回/过滤过严等）。仪表盘有「查看诊断详情」。
- **宽松模式**：放宽成交额/RSI/强势硬条件，舆情/资金缺失显示 N/A 且不排除股票（`--lenient` 或页面勾选）。
- 状态栏显示股票池来源(hs300/zz500/all/watchlist/cache/default)、原始/过滤后数量、缓存路径与更新时间。
- 诊断 API：`GET /api/universe/status?universe=hs300`。
- 默认快速池为约 60 只**真实** A 股（各行业代表），仅用于快速演示；真正全市场请选 hs300/zz500/all。

> 若运行报「行情数据未返回」：成分股已取到，但价格接口失败/网络问题——先点「🔍 数据源检测」排查 Baostock/AkShare。
